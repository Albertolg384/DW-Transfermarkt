...
def etl_dim_clubs():
    """Extrae, transforma y carga la dimension de clubes"""
    print("ETL: dim_clubs")
    print("=" * 60)
    
    # EXTRACT
    print("Extrayendo clubs.csv...")
    ...
    # Limpiar valores numericos (total_market_value puede ser string con "€")
    ...
    # Validacion: asegurar que club_id no sea nulo.
    nulls = dim_clubs['club_id'].isna().sum()
    if nulls > 0:
        print(f" {nulls} registros con club_id NULL eliminados")
    ...
    # LOAD
    print("Cargando a PostgreSQL (dwh.dim_clubs)...")
    engine = get_engine()
    ...
    with engine.connect() as conn:
        ...
        """))
        conn.commit()
    print("Registro centinela (club_id=-1, 'Desconocido') insertado")
    ...
    with engine.connect() as conn:
        ...
        print(f"Verificacion: {count:,} registros en dwh.dim_clubs\n")
