...
"""
Comparacion entre partidos FILTRADOS vs VALIDOS
Para entender por que unos si y otros no.
"""
import pandas as pd
...
print("=" * 100)
print("COMPARACION: PARTIDOS FILTRADOS vs PARTIDOS VALIDOS")
print("=" * 100)
...
# Conectar a PostgreSQL para ver que quedo en dim_games.
...
# Partidos VALIDOS (ambos clubs existen).
...
print("\n" + "=" * 100)
print("EJEMPLOS DE PARTIDOS VALIDOS (SI cargados en DWH)")
print("=" * 100)
...
for idx, game in valid_games.iterrows():
    ...
    print(f"  ")
    print(f"AWAY: {game['away_club_name']} (ID: {int(game['away_club_id']) if pd.notna(game['away_club_id']) else 'NULL'})")
    ...
    print(f"Razon: Ambos clubs existen --> PASA el filtro")
...
for idx, game in filtered_games.iterrows():
    ...
    if pd.notna(game['home_club_id']):
        ...
        print(f"home_club_id es NULL")
    
    print(f"  ")
    print(f"AWAY: {game['away_club_name'] if pd.notna(game['away_club_name']) else ' DESCONOCIDO'} (ID: {int(game['away_club_id']) if pd.notna(game['away_club_id']) else 'NULL'})")
    ...
    print(f"Razon: {' Y '.join(reasons)} --> NO pasa el filtro")
...
print(f" ")
print(f"Ambos clubs validos: {games_both_valid:,} ({100*games_both_valid/total_games:.1f}%)")
print(f"Home club invalido: {games_home_invalid:,} ({100*games_home_invalid/total_games:.1f}%)")
print(f"Away club invalido: {games_away_invalid:,} ({100*games_away_invalid/total_games:.1f}%)")
print(f"Ambos clubs invalidos: {games_both_invalid:,} ({100*games_both_invalid/total_games:.1f}%)")
...
print("""
PARTIDOS VALIDOS tienen:
   - home_club_id pertenece clubs.csv.
   - away_club_id pertenece clubs.csv. 
   - Se pueden cargar en dim_games sin problemas.
   - Sus JOIN funcionan correctamente.

PARTIDOS FILTRADOS tienen:
   - Al menos un club_id que NO existe en clubs.csv.
   - No se pueden crear FKs validas a dim_clubs.
   - Si se cargaran, los JOIN fallarian.
   - Producirian datos incompletos en reports.
