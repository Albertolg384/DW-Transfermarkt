...
def reset_dwh():
    ...
    print("=" * 70)
    print("ADVERTENCIA: RESETEAR DATA WAREHOUSE")
    print("=" * 70)
    print("\nEsto BORRARA todos los datos de las siguientes tablas:")
    print("- Todas las tablas de hechos (fact_*)")
    print("- Todas las tablas de dimensiones (dim_*)")
    print("\nLa estructura (tablas, columnas, indices) se mantiene.")
    print("\nEsta operacion NO SE PUEDE DESHACER sin backup.\n")
    
    response = input("¿Estas SEGURO de continuar? Escribe 'BORRAR' para confirmar: ")
    
    if response != 'BORRAR':
        print("Operacion cancelada. Ningun dato fue borrado.")
        return
    ...
    try:
        with engine.connect() as conn:
            ...
            print("   Borrando tablas de hechos...")
            tables_fact = [
                'fact_game_events',
                'fact_appearances',
                'fact_transfers',
            ...
            # Borrar dimensiones
            print("Borrando tablas de dimensiones...")
            tables_dim = [
                'dim_games',
                'dim_players',
                'dim_clubs',
        ...
        print("Todas las tablas estan vacias pero la estructura se mantiene.")
        print("Puedes ejecutar el ETL nuevamente con: python run_etl_full.py\n")
        
    except Exception as e:
        print(f"\n ERROR durante el reseteo: {e}")
        print("Verifica la conexion a PostgreSQL y los permisos.")
        sys.exit(1)
