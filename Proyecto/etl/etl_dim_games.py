...
"""
ETL para dim_games (tabla de partidos como dimension)
"""
import pandas as pd
...
def etl_dim_games():
    """Extrae, transforma y carga la dimension de partidos"""
    print("ETL: dim_games")
    print("=" * 60)
    
    # EXTRACT
    print("Extrayendo games.csv...")
    ...
    # Truncar campos VARCHAR que pueden exceder limites del DDL.
    varchar_limits = {
    ...
    # Validacion: registros con game_id, competition_id, date validos.
    critical_nulls = dim_games[['game_id', 'competition_id', 'date']].isna().any(axis=1).sum()
    if critical_nulls > 0:
        print(f"{critical_nulls} registros con campos criticos NULL eliminados")
        dim_games = dim_games[
    ...
    # Validacion final de NULLs
    validate_no_nulls(dim_games, 'dim_games')

    print(f"{len(dim_games):,} registros listos para carga")
        
    # LOAD
    print("Cargando a PostgreSQL (dwh.dim_games)...")
    
    # Insertar en chunks mas pequenios para evitar problemas de parametros.
    chunk_size = 1000  # Reducido para esta tabla por el numero de columnas.
    ...
    for i in range(0, len(dim_games), chunk_size):
        ...
        chunk.to_sql(
            ...
            if_exists='append',
            index=False,
            method=None, # Usar metodo por defecto en lugar de 'multi'.
        )
    ...
    with engine.connect() as conn:
        ...
        print(f"Verificacion: {count:,} registros en dwh.dim_games\n")
