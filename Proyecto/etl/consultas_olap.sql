...
-- |        Esquema en Constelacion Kimball | PostgreSQL 18                       |
-- |                                                                              |
-- |  Modelo:                                                                     |
-- |    Dimensiones: dim_date, dim_competitions, dim_clubs, dim_players,          |
-- |                 dwh.dim_games                                                |
-- |    Hechos: fact_games, fact_appearances, fact_game_events,                   |
-- |            fact_player_valuations, dwh.fact_transfers                        |
-- |                                                                              |
-- |  Clasificacion de operaciones OLAP:                                          |
-- |    1. SLICE - Corte por una dimension                                        |
-- |    2. DICE - Subcubo multidimensional                                        |
-- |    3. ROLL-UP - Agregacion ascendente (detalle --> resumen)                  |
-- |    4. DRILL-DOWN - Desglose descendente (resumen --> detalle)                |
-- |    5. PIVOT - Rotacion de ejes (tabulacion cruzada)                          |
...
-- 1. SLICE (CORTE)
-- ============================================================================
-- Operacion que fija UNA dimension en un valor concreto y muestra el resto
...
-- ---------------------------------------------------------------------------
-- Se fija la dimension TIEMPO en transfer_season = 2023 y se analiza
-- el volumen y coste de fichajes por club de destino.
-- Util para: "¿Que clubes invirtieron mas en el mercado de verano 2023?"
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- 1.2  SLICE POR COMPETICION - Rendimiento de equipos en la Premier League
-- ---------------------------------------------------------------------------
-- Se fija la dimension COMPETICION en 'GB1' (Premier League) y se analiza
-- el rendimiento goleador local vs visitante de cada club.
-- Util para: "¿Que equipo es mas fuerte en casa en la Premier?"
-- ---------------------------------------------------------------------------
SELECT
...
WHERE fg.competition_id = 'GB1' -- SLICE: fija competicion (Premier League)
...
-- ---------------------------------------------------------------------------
-- Se fija la dimension CLUB y se muestra cada transferencia de entrada/salida
...
-- 2. DICE (SUBCUBO)
-- ============================================================================
-- Operacion que recorta el cubo filtrando por DOS O MAS dimensiones
-- simultaneamente. El resultado es un "subcubo" mas pequeño.
-- ============================================================================
...
--   - COMPETICION: las Big Five (ES1, GB1, IT1, FR1, L1)
--   - TIEMPO: temporadas 2020 a 2024
-- Util para: "¿Quien es el maximo goleador español en Europa en los ultimos 5 años?"
-- ---------------------------------------------------------------------------
SELECT
    p.name AS jugador,
...
-- Util para: "¿Cuanto talento exporta Alemania a Inglaterra?"
-- ---------------------------------------------------------------------------
SELECT
...
-- ============================================================================
-- 3. ROLL-UP (AGREGACION ASCENDENTE)
-- ============================================================================
-- Operacion que navega de un nivel de granularidad fino a uno mas grueso,
-- colapsando una o mas dimensiones. Se "sube" en la jerarquia dimensional.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Partiendo del grano mas fino (mes), se agregan los goles por trimestre,
-- luego por año, y finalmente el gran total. Usa ROLLUP() de SQL.
-- Jerarquia: mes --> trimestre --> año --> total
-- ---------------------------------------------------------------------------
SELECT
    d.year AS anio,
...
GROUP BY ROLLUP(d.year, d.quarter, d.month) -- ROLL-UP jerarquico
ORDER BY d.year NULLS LAST, d.quarter NULLS LAST, d.month NULLS LAST;


-- ---------------------------------------------------------------------------
-- 3.2  ROLL-UP GEOGRAFICO - Ingresos por transferencias:
--       Club --> Liga --> Confederacion --> Total
-- ---------------------------------------------------------------------------
-- Jerarquia geografica: club individual --> liga nacional --> confederacion continental
-- Muestra cuanto genero cada nivel vendiendo jugadores.
-- ---------------------------------------------------------------------------
SELECT
...
GROUP BY ROLLUP(comp.confederation, comp.name, co.name) -- ROLL-UP geografico
...
-- ---------------------------------------------------------------------------
-- 3.3  ROLL-UP DEPORTIVO - Goleadores: Jugador --> Pais --> Posicion --> Total
-- ---------------------------------------------------------------------------
-- Jerarquia: jugador individual --> su nacionalidad --> su posicion --> gran total
...
-- ============================================================================
-- Operacion inversa al Roll-Up. Se parte de un nivel agregado y se
-- descompone en niveles mas detallados ("bajar" en la jerarquia).
-- ============================================================================
...
-- Util para: "¿En que meses hay mas publico en la Premier?"
-- ---------------------------------------------------------------------------
...
--       Maximos goleadores de La Liga 2022/23
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4.3  DRILL-DOWN DE VALOR DE MERCADO - De confederacion a liga a club a jugador
-- ---------------------------------------------------------------------------

-- Nivel 1: Valor total de mercado por CONFEDERACION
SELECT
    'Nivel 1: Confederacion' AS nivel,
    comp.confederation,
...
FROM dwh.fact_player_valuations pv
    ...
    INNER JOIN dwh.dim_competitions comp ON pv.competition_id = comp.competition_id
WHERE d.year = 2024 AND d.month = 1
  AND comp.confederation = 'europa' -- DRILL-DOWN: fija confederacion
GROUP BY comp.name
ORDER BY valor_total DESC NULLS LAST
LIMIT 10;
...
-- ============================================================================
-- 5. PIVOT (ROTACION / TABULACION CRUZADA)
-- ============================================================================
-- Operacion que rota los ejes del cubo para presentar los datos en formato
-- de tabla cruzada (una dimension en filas, otra en columnas).
...
-- ---------------------------------------------------------------------------
-- 5.2  PIVOT - Fichajes por club de destino y temporada (ultimas 5 temporadas)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5.3  PIVOT - Distribucion de eventos por tipo y dia de la semana
-- ---------------------------------------------------------------------------
-- Filas: tipo de evento (Goals, Cards, Substitutions)
-- Columnas: dias de la semana
-- Util para: "¿Hay mas goles los sabados que entre semana?"
-- ---------------------------------------------------------------------------
SELECT
...
-- ============================================================================
-- Funciones analiticas que operan sobre un conjunto de filas relacionadas
-- (la "ventana") sin colapsar el resultado. Incluyen:
-- - RANK / DENSE_RANK / ROW_NUMBER --> Rankings
-- - SUM() OVER / AVG() OVER  --> Acumulados y medias moviles
-- - LAG / LEAD --> Comparaciones con periodo anterior/siguiente
-- - NTILE --> Distribucion en cuantiles
-- ============================================================================
...
-- Muestra como crece el gasto total del club a lo largo de los años.
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- 6.2  MEDIA MOVIL - Promedio de goles por partido (media movil de 3 meses)
--                     en La Liga
-- ---------------------------------------------------------------------------
-- AVG() OVER con ROWS BETWEEN crea una media movil suavizada.
...
-- ---------------------------------------------------------------------------
-- 6.3  LAG - Variacion año a año del valor de mercado de Mbappe
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6.4  NTILE - Distribucion de jugadores en cuartiles por goles (Tier System)
-- ---------------------------------------------------------------------------
-- NTILE(4) divide a los jugadores en 4 cuartiles segun su produccion goleadora.
...
-- ---------------------------------------------------------------------------
-- 6.5  RANKING POR PARTICION - Mejor goleador de CADA club en CADA liga
-- ---------------------------------------------------------------------------
-- ROW_NUMBER() particionado por (liga, club) encuentra al maximo goleador
...
-- 7. GROUPING SETS / CUBE / ROLLUP (AGREGACIONES MULTINIVEL)
-- ============================================================================
-- Extensiones SQL que generan MULTIPLES niveles de agregacion en una sola
-- consulta, evitando UNIONs manuales.
-- - GROUPING SETS: niveles especificos elegidos manualmente
-- - CUBE: todas las combinaciones posibles de dimensiones
-- - ROLLUP: jerarquia de mayor a menor detalle (ya visto, aqui con variantes)
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Genera 4 niveles de agregacion simultaneos sin UNION ALL:
--   1. Por cada combinacion liga + temporada
...
-- ---------------------------------------------------------------------------
-- 7.2  CUBE - Analisis multidimensional completo de apariciones:
--             Posicion × Tipo de aparicion × Capitan
-- ---------------------------------------------------------------------------
-- CUBE genera TODAS las combinaciones posibles de las 3 dimensiones:
--   (posicion, tipo, capitan), (posicion, tipo), (posicion, capitan),
--   (tipo, capitan), (posicion), (tipo), (capitan), ()
-- Total: 2^3 = 8 niveles de agregacion en una consulta.
-- ---------------------------------------------------------------------------
SELECT
...
--                       con subtotales por confederacion
-- ---------------------------------------------------------------------------
-- Combina compras y ventas en una sola consulta para calcular el
-- balance neto del mercado de fichajes por liga y confederacion.
-- ---------------------------------------------------------------------------
WITH compras AS (
    SELECT
        comp.confederation,
...
SELECT
    ...
    CASE 
        WHEN COALESCE(v.ingreso_total, 0) - COALESCE(c.gasto_total, 0) > 0 THEN 'SUPERAVIT'
        WHEN COALESCE(v.ingreso_total, 0) - COALESCE(c.gasto_total, 0) < 0 THEN 'DeFICIT'
        ELSE 'EQUILIBRIO'
    END                                             AS estado
...
-- ============================================================================
-- 8. ANALISIS ESTRATEGICOS COMBINADOS
-- ============================================================================
-- Consultas complejas que combinan multiples operaciones OLAP para
-- casos de estudio especificos. Utiles para storytelling y analisis profundo.
-- Combinan: SLICE + WINDOW + LAG + PIVOT + CTEs + agregaciones avanzadas.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- 8.1  SLICE + WINDOW - Evolucion del valor de mercado del Top 5 jugadores
--                        mas valiosos de La Liga, con media movil
-- ---------------------------------------------------------------------------
-- Combina: SLICE (La Liga) + RANKING (top 5) + MEDIA MOVIL (6 meses)
-- ---------------------------------------------------------------------------
...
--       ¿Los jugadores que entran de cambio marcan mas por minuto?
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8.3  DRILL-DOWN + LAG - Analisis interanual de fichajes con variacion
--       porcentual por liga
-- ---------------------------------------------------------------------------
-- Combina: Drill-down temporal (temporada) + LAG (comparar con año anterior)
-- Responde: "¿Que liga incremento mas su gasto en fichajes año a año?"
-- ---------------------------------------------------------------------------
SELECT
    liga,
    temporada,
...
-- ---------------------------------------------------------------------------
-- 8.4  CUBE + RANKING - Analisis completo de tarjetas:
--       Dimensiones posicion × liga × dia_semana con TOP infractores
-- ---------------------------------------------------------------------------
-- Parte 1: CUBE de tarjetas amarillas por posicion y liga
-- Parte 2: TOP 10 jugadores mas amonestados
-- ---------------------------------------------------------------------------
...
-- Parte 2: TOP 10 jugadores mas amonestados (ranking)
SELECT
...
-- Cada consulta combina multiples operaciones OLAP (SLICE + WINDOW + DICE)
-- para responder preguntas analiticas concretas del negocio.
-- 
-- Incluye recomendaciones de visualizacion para la presentacion.
-- Cada consulta tiene un insight clave esperado.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- 9.1 TOP 25 GOLEADORES HISTORICOS DE LAS 5 GRANDES LIGAS
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Quien ha marcado mas goles en las Big Five desde que tenemos datos?"
-- Operaciones OLAP: SLICE (Big Five) + WINDOW (ROW_NUMBER) + Agregaciones
-- Visualizacion: Grafico de barras horizontal con banderas nacionalidad
-- Insight esperado: Lewandowski domina, seguido Messi/Benzema. 
--    Posicion Attack representa >90% del top 25.
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- 9.2  TOP 25 MAXIMOS ASISTENTES DE LAS BIG FIVE
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Quien ha dado mas asistencias en las 5 grandes ligas?"
-- Operaciones OLAP: SLICE (Big Five) + WINDOW (ROW_NUMBER) + Agregaciones
-- Visualizacion: Grafico de barras horizontal, destacar midfielders
...
-- ---------------------------------------------------------------------------
-- 9.3  TOP 20 FICHAJES MAS CAROS DE LA HISTORIA (en el dataset)
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Cuales han sido las transferencias mas caras?"
-- Operaciones OLAP: WINDOW (ROW_NUMBER) + Calculos derivados (ratio, valoracion)
-- Visualizacion: Tabla con colores segun valoracion (rojo=sobrepagado, verde=ganga)
-- Insight esperado: Mbappe #1. Muchos fichajes ingleses sobrepagados.
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Gana mas el equipo local? ¿En que liga es mas fuerte el factor campo?"
-- Operaciones OLAP: SLICE (Big Five) + Agregaciones condicionales + PIVOT-like
-- Visualizacion: Grafico de columnas apiladas 100% (Local/Empate/Visitante)
-- Insight esperado: ~45% victorias local, ~27% empates. Bundesliga mas ofensiva.
-- ---------------------------------------------------------------------------
SELECT
    comp.name                                       AS liga,
...
-- ---------------------------------------------------------------------------
-- 9.5 PICHICHI POR TEMPORADA - Maximo goleador de cada liga cada año
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Quien fue el Pichichi/Capocannoniere/Torjäger cada temporada?"
-- Operaciones OLAP: SLICE + DRILL-DOWN (liga-->temporada) + WINDOW (RANK)
-- Visualizacion: Heatmap o matriz (ligas en columnas, años en filas, foto jugador)
...
-- ---------------------------------------------------------------------------
-- 9.6 EL CLUB QUE MAS GASTA vs EL QUE MAS GANA EN FICHAJES (Big Five)
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que club tiene el mayor deficit y cual el mayor superavit 
--            en el mercado de fichajes?"
-- Operaciones OLAP: DICE (Big Five) + CTEs + FULL OUTER JOIN + Balance neto
-- Visualizacion: Grafico de barras divergentes (deficit izq, superavit der)
-- Insight esperado: Chelsea mayor deficit (-1.591M€). Clubes menores venden.
-- ---------------------------------------------------------------------------
WITH gastos AS (
    SELECT
        c.name AS club,
...
-- ---------------------------------------------------------------------------
-- 9.7 ¿EN QUE MINUTO SE MARCAN MAS GOLES? - Distribucion por franja
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Se marcan mAs goles al final de cada parte?"
-- Operaciones OLAP: SLICE (Big Five) + PIVOT (franjas minuto) + Agregacion
-- Visualizacion: GrAfico de Area o histograma con picos destacados
...
-- ---------------------------------------------------------------------------
-- 9.8  TOP 10 NACIONALIDADES MAS GOLEADORAS EN LAS BIG FIVE
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que pais produce mas goleadores en las grandes ligas europeas?"
-- Operaciones OLAP: DICE (Big Five + nacionalidad) + WINDOW + Agregaciones
-- Visualizacion: Mapa de calor mundial o barras con bandera de pais
...
-- ---------------------------------------------------------------------------
-- 9.9 INFLACION DEL MERCADO - Precio medio de fichaje por año
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Los fichajes son cada vez mas caros? ¿Cuanto ha crecido el precio?"
-- Operaciones OLAP: ROLL-UP (temporal) + WINDOW (LAG) + PERCENTILE_CONT (mediana)
-- Visualizacion: Grafico de lineas con 2 ejes (precio medio + % variacion)
-- Insight esperado: Crecimiento exponencial hasta 2019. Caida COVID 2020-21.
-- ---------------------------------------------------------------------------
WITH base AS (
    SELECT
        d.year                                      AS anio,
...
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Cual es la liga con mas goles, mas publico, mas fichajes?"
-- Operaciones OLAP: DICE (Big Five) + Multiples CTEs + JOIN de hechos diferentes
-- Visualizacion: Tabla resumen multicolor o radar chart (5 ligas, 6 KPIs)
-- Insight esperado: Premier mas goles y asistencia. Bundesliga menos tarjetas.
-- ---------------------------------------------------------------------------
WITH goles_liga AS (
    SELECT
        fg.competition_id,
...
--   - Analisis combinados (8): 5 consultas
--   - Consultas estrella presentables (9): 10 consultas
--   - Apendice tecnico (10): 1 consulta de auditoria
-- ============================================================================


-- ============================================================================
-- 10. APENDICE TECNICO - AUDITORIA Y CALIDAD DE DATOS
-- ============================================================================
-- Consultas de validacion tecnica del Data Warehouse.
-- NO son analisis de negocio, sino verificaciones de arquitectura Kimball.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- 10.1  AUDITORIA DE REGISTROS CENTINELA (Unknown Member Rows)
-- ---------------------------------------------------------------------------
-- Verifica el funcionamiento del patron Kimball de registros centinela (-1).
-- Muestra quee porcentaje de datos usa el centinela vs registros reales.
-- 
-- Util para: Validar la estrategia de NULLs y demostrar que NO hay NULLs
...
-- ============================================================================
-- 11. ANALISIS INDIVIDUAL DE JUGADORES
-- ============================================================================
-- Consultas centradas en el perfil, evolucion y comparativa de jugadores.
-- Combinan fact_appearances, fact_player_valuations y fact_transfers para
-- construir una vision 360° del rendimiento individual.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- 11.1  SLICE + WINDOW - TOP 20 jugadores con mas apariciones historicas
--                         en las Big Five
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que jugadores han sido mas utilizados a lo largo de la historia?"
-- Operaciones OLAP: SLICE (Big Five) + WINDOW (ROW_NUMBER, NTILE) +
--                   Agregaciones (minutos, goles, partidos)
-- Visualizacion: Barras horizontales con foto jugador y porcentaje de titularidades
-- Insight esperado: Porteros y defensas dominan por longevidad.
--   Los centrocampistas acumulan mas minutos totales que los delanteros.
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- 11.2  DICE + PIVOT - Rendimiento como capitan vs no capitan
--                       ¿Rinden mejor los jugadores con el brazalete?
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Marcan y asisten mas los jugadores cuando son capitanes?"
-- Operaciones OLAP: DICE (Big Five + posicion) + PIVOT (capitan si/no en columnas)
--                 + WINDOW (RANK por posicion)
-- Visualizacion: Tabla comparativa lado a lado con delta y % cambio
-- Insight esperado: Los capitanes tienen mejor rendimiento. Delanteros con
--   brazalete marcan ~15% mas por partido que sin el (efecto responsabilidad).
-- ---------------------------------------------------------------------------
WITH rendimiento AS (
    SELECT
        p.position                                                  AS posicion,
...
SELECT
    ...
    -- Tarjetas: ¿mas responsabilidad = mas disciplina?
...
-- ---------------------------------------------------------------------------
-- 11.3  DICE + WINDOW - Jugadores que han jugado en mas clubes distintos
--                        ("los mas nomadas del futbol europeo")
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que jugadores han pasado por mas equipos diferentes?"
-- Operaciones OLAP: DICE (Big Five + rango temporal) + WINDOW (DENSE_RANK)
--                 + STRING_AGG para listar los clubes
-- Visualizacion: Treemap o tabla con linea de tiempo de clubes
-- Insight esperado: Jugadores con poca continuidad ≠ mal rendimiento.
--   Muchos nomadas son centrocampistas versatiles buscados por multiples clubes.
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- 11.4  WINDOW (LAG + LEAD) - Jugadores que mas aumentaron su valor de mercado
--                              en un solo año (mayores revalorizaciones)
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que jugadores pegaron el mayor salto de valor en un año?"
-- Operaciones OLAP: WINDOW (LAG para año anterior, LEAD para año siguiente)
--                 + SLICE (ultimo valor conocido vs maximo)
-- Visualizacion: Grafico de flechas ascendentes (de --> a), ordenado por % subida
-- Insight esperado: Jugadores Sub-23 con temporada revelacion.
--   La mayor revalorizacion en 1 año suele triplicar el valor previo.
-- ---------------------------------------------------------------------------
...
--                       ¿Los zurdos son mas productivos que los diestros?
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Hay diferencias estadisticas de rendimiento entre zurdos y diestros?"
-- Operaciones OLAP: DICE (Big Five + posicion) + PIVOT (pie como columnas)
--                 + Agregaciones por 90 min para comparar equitativamente
-- Visualizacion: Radar chart (goles/90, asist/90, min jugados, amarillas/90)
-- Insight esperado: Los zurdos son mas escasos (~20%) pero con rendimiento
...
-- ============================================================================
-- 12. ANALISIS DE EQUIPOS, COMPETICION Y TACTICA
-- ============================================================================
-- Consultas enfocadas en el rendimiento colectivo, patrones de juego por equipo
-- y dinamica de competiciOn. Crucian fact_games con dim_clubs y dim_date.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- 12.1  WINDOW (RANK) - Mejores ataques y defensas historicas de cada liga
--                        (Top 5 mas goleadores + Top 5 menos goleados)
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que equipo ha tenido el mejor ataque y la mejor defensa de la historia
--            en cada liga?"
-- Operaciones OLAP: SLICE (por liga) + WINDOW (DENSE_RANK por particion)
--                 + Agregaciones (goles a favor, en contra, diferencia)
-- Visualizacion: Tabla doble ranking por liga, color verde ataque / azul defensa
-- Insight esperado: PSG lidera ataque en Ligue 1. Atletico Madrid mejor defensa
--   historica en La Liga. Bayern en Bundesliga domina ambas categorias.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 12.2  DICE + CTE - ¿Gastar mas en fichajes se traduce en mas victorias?
--                     Correlacion entre inversion y rendimiento deportivo
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Los equipos que mas gastan en fichajes ganan mas partidos?"
-- Operaciones OLAP: DICE (Big Five + periodo 2018-2024) + JOIN entre
--                   fact_transfers y fact_games + WINDOW (NTILE para cuartiles)
-- Visualizacion: Scatter plot gasto vs % victorias, con linea de tendencia
-- Insight esperado: Correlacion positiva moderada (r ≈ 0.6). Los 3 clubes con
...
-- ---------------------------------------------------------------------------
-- 12.3  ROLL-UP + WINDOW - ¿Cual fue la temporada mas goleadora de cada liga?
--                           Con comparativa respecto a la media historica
-- ---------------------------------------------------------------------------
-- Pregunta: "¿En que temporada se marcaron mas goles? ¿Que liga ha sido mas
--            estable en su produccion goleadora?"
-- Operaciones OLAP: ROLL-UP (temporada dentro de liga) + WINDOW (LAG, RANK,
--                   AVG historico como referencia)
-- Visualizacion: Lineas de evolucion por liga con banda de desviacion estandar
-- Insight esperado: Post-COVID (2021/22) hubo un repunte goleador.
--   Bundesliga la mas volatil, Serie A la mas estable.
-- ---------------------------------------------------------------------------
...
SELECT
    ...
    goles_totales,
    promedio_goles,
    -- Media historica de esa liga (para comparar)
...
-- ---------------------------------------------------------------------------
-- 12.4  DICE + RANKING - Los 25 partidos con mas goles de la historia
--                         en las 5 grandes ligas
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Cuales han sido los partidos mas locos y espectaculares?"
-- Operaciones OLAP: DICE (Big Five) + WINDOW (ROW_NUMBER) +
--                   Calculo de diferencia y tipo de resultado
-- Visualizacion: Tarjetas de partido estilo marcador con escudo y fecha
-- Insight esperado: Varios partidos con 9-10 goles totales. La Bundesliga
--   esta sobrerrepresentada (estilo mas ofensivo). Muchos no terminan en empate.
-- ---------------------------------------------------------------------------
SELECT
...
-- ---------------------------------------------------------------------------
-- Pregunta: "¿Que equipos mejoran en la segunda vuelta? ¿Cuales bajan el ritmo?"
...
-- Visualizacion: Tabla con flechas ↑↓ segun mejora/empeora en 2ª vuelta
-- Insight esperado: Equipos que fichan en enero mejoran en 2ª vuelta.
--   Los campeones suelen tener rendimiento homogeneo en ambas vueltas.
-- ---------------------------------------------------------------------------
