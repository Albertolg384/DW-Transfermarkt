...
"""
Validación COMPLETA del ETL del Data Warehouse
Verifica:
1. Comparación de conteos CSV vs PostgreSQL
2. Validación de medidas calculadas
...
def validate_full_etl():
    """Ejecuta validación completa contra CSVs originales"""
    print("=" * 80)
    print("🔍 VALIDACIÓN COMPLETA DEL ETL - DATA WAREHOUSE TRANSFERMARKT")
    print("=" * 80)
    
    engine = get_engine()
    
    # ============================================================
    # 1. COMPARACIÓN CSV vs POSTGRESQL
    # ============================================================
    print("\n1️⃣ COMPARACIÓN DE CONTEOS: CSV ORIGINALES vs POSTGRESQL")
    print("-" * 80)
    ...
    if db_games < csv_games:
        print(f"      ⚠️ {csv_games - db_games:,} partidos filtrados (clubs/competitions inexistentes)")
    ...
    # fact_games (desnormalización de games + club_games)
    ...
    # ============================================================
    # 2. VALIDACIÓN DE MEDIDAS CALCULADAS (LAB-BOOK)
    # ============================================================
    print("\n\n2️⃣ VALIDACIÓN DE MEDIDAS CALCULADAS (según lab-book)")
    print("-" * 80)
    ...
    if db_fact_games > 0:
        ...
        print(f"\n   ✅ Validación de cálculos sobre {total:,} partidos:")
        ...
        print("\n   ⚠️ TABLA VACÍA - Ejecuta primero el ETL de fact_games")
    ...
    if db_appearances > 0:
        ...
        print("   ⚠️ TABLA VACÍA - Ejecuta primero el ETL de fact_appearances")
    ...
    if db_events > 0:
        ...
        print("   ⚠️ TABLA VACÍA - Ejecuta primero el ETL de fact_game_events")
    
    # fact_transfers - PK artificial
    print("\n\n🔄 fact_transfers - Verificación de PK artificial (transfer_id):")
    if db_transfers > 0:
        ...
        if transfers_pk.iloc[0, 2] == transfers_pk.iloc[0, 3]:
            print("   ✅ Todos los transfer_id son únicos (PK válida)")
    else:
        print("   ⚠️ TABLA VACÍA - Ejecuta primero el ETL de fact_transfers")
    
    # fact_player_valuations - PK artificial
    print("\n\n💰 fact_player_valuations - Verificación de PK artificial (valuation_id):")
    if db_valuations > 0:
        ...
        if valuations_pk.iloc[0, 2] == valuations_pk.iloc[0, 3]:
            print("   ✅ Todos los valuation_id son únicos (PK válida)")
    else:
        print("   ⚠️ TABLA VACÍA - Ejecuta primero el ETL de fact_player_valuations")
    
    # ============================================================
    # 3. INTEGRIDAD REFERENCIAL (FK)
    # ============================================================
    print("\n\n3️⃣ VALIDACIÓN DE INTEGRIDAD REFERENCIAL (Foreign Keys)")
    print("-" * 80)
    ...
    for check_name, query in fk_checks:
        ...
        if orphans == 0:
            print(f"   ✅ {check_name}: 0 huérfanos")
        else:
            print(f"   ❌ {check_name}: {orphans} huérfanos")
            fk_ok = False
    
    # ============================================================
    # 4. DESNORMALIZACIÓN VERIFICADA
    # ============================================================
    print("\n\n4️⃣ VERIFICACIÓN DE DESNORMALIZACIÓN")
    print("-" * 80)
    ...
    if db_appearances > 0:
        ...
        print("\n\n👕 fact_appearances: ⚠️ TABLA VACÍA")
    ...
    # Detectar si las tablas de hechos están vacías
    facts_empty = (db_fact_games == 0 and db_appearances == 0 and 
                   db_events == 0 and db_transfers == 0 and db_valuations == 0)
    
    if facts_empty:
        print("\n" + "⚠️" * 40)
        print("⚠️  LAS TABLAS DE HECHOS ESTÁN VACÍAS")
        print("⚠️" * 40)
        print("\n🔧 SIGUIENTE PASO: Ejecutar el ETL completo:")
        print("   cd \"c:\\Users\\manuc\\OneDrive\\Escritorio\\Futbol_Dataset (1)\\etl\"")
        print("   python run_etl_full.py")
        print("\n   Esto cargará las 5 tablas de hechos con ~3.4 millones de registros")
        print("   Tiempo estimado: 10-30 minutos")
        return
    
    if total_db/total_csv < 0.95:
        print(f"\n⚠️ NOTA: Se filtraron {total_csv - total_db:,} registros ({(1-total_db/total_csv)*100:.1f}%)")
        print("   Razones:")
        print("   - Registros con FK inválidas (clubs/competitions/players inexistentes)")
        print("   - Registros con valores NULL en campos críticos")
        print("   - Duplicados por PK")
        print("   - Partidos con clubs fuera del dataset (dim_games filtró 15,540)")
    ...
    print(f"✅ Desnormalización: APLICADA (fact_games, fact_appearances)")
    print(f"✅ Dimensión Fecha: GENERADA (2000-2030, {db_date:,} registros)")
